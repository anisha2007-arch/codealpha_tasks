...
const Chrome = (() => {
  ...
  async function renderHeader() {
    ...
    host.innerHTML = `
      <header class="topbar">
        <div class="bar-inner">
          <a class="wordmark" href="/">Roundtable</a>
          <div class="bar-right">
            ${user ? `
              <span class="who">${Html.avatar(user, 'tiny')} ${Html.escape(user.name)}</span>
    ...
    // it — the notifications badge — waits for this rather than for
    // DOMContentLoaded, which has long since fired.
